...
from .exceptions import FileNotFoundOrInvalidURLError
...
class Bot:
    ...
    def send_photo(self, chat_id, photo: str, caption):
        try:
            if os.path.exists(photo):
                with open(photo, 'rb') as p:
                    httpx.post(self.api_url+"sendPhoto", data={"chat_id": chat_id, 'caption': caption}, files={"photo": p})
            elif "https://" in photo:
                httpx.post(self.api_url+"sendPhoto", json={"chat_id": chat_id, "photo": photo, 'caption': caption})
        except:
            raise FileNotFoundOrInvalidURLError(f"Photo not found or invalid URL: {photo}")

    def send_video(self, chat_id, video: str, caption):
        try:
            if os.path.exists(video):
                with open(video, 'rb') as v:
                    httpx.post(self.api_url+"sendVideo", data={"chat_id": chat_id, 'caption': caption}, files={"video": v})
            elif "https://" in video:
                httpx.post(self.api_url+"sendVideo", json={"chat_id": chat_id, "video": video, 'caption': caption})
        except:
            raise FileNotFoundOrInvalidURLError(f"Video not found or invalid URL: {video}")

    def send_audio(self, chat_id, audio: str, caption):
        try:
            if os.path.exists(audio):
                with open(audio, 'rb') as a:
                    httpx.post(self.api_url+"sendAudio", data={"chat_id": chat_id, 'caption': caption}, files={"audio": a})
            elif "https://" in audio:
                httpx.post(self.api_url+"sendAudio", json={"chat_id": chat_id, "audio": audio, 'caption': caption})
        except:
            raise FileNotFoundOrInvalidURLError(f"Audio not found or invalid URL: {audio}")
    
    def send_voice(self, chat_id, voice: str, caption):
        try:
            if os.path.exists(voice):
                with open(voice, 'rb') as v:
                    httpx.post(self.api_url+"sendVoice", data={"chat_id": chat_id, 'caption': caption}, files={"voice": v})
        except:
    ...
    def process_messages(self, message):
        ...
        if isinstance(handled['text'], Photo):
            self.send_photo(chat_id, handled['text'].url, caption=handled['text'].caption)
        elif isinstance(handled['text'], Video):
            self.send_video(chat_id, handled['text'].url, caption=handled['text'].caption)
        elif isinstance(handled['text'], Audio):
            self.send_audio(chat_id, handled['text'].url, caption=handled['text'].caption)
        elif isinstance(handled['text'], Voice):
            self.send_voice(chat_id, handled['text'].url, handled['text'].caption)
